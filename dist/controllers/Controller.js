    ...
    registration(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const candidate = yield user_model_1.userModel.findOne({ username });
                if (candidate) {
                    return res.status(400).json({ message: "Username ֊ը զբաղված է։" });
                }
                const hashedPasswd = bcrypt_1.default.hashSync(password, 10);
                const user = new user_model_1.userModel({ name, lastName, phoneNumber, email, username, password: hashedPasswd });
                yield user.save();
